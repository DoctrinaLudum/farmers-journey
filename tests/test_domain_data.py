...
@pytest.mark.parametrize("item_id, data", wearablesItemBuffs.WEARABLES_DATA.items())
def test_wearable_item_structure_and_values(item_id, data):
    """
    Testa cada item no dicionário WEARABLES_DATA para garantir que:
    1. A estrutura de chaves está correta e os IDs são consistentes.
    2. Os tipos de dados são os esperados (int, str, bool, etc.).
    3. Os valores (como 'part') são válidos e o caminho da imagem existe.
    4. A estrutura dos boosts está correta e os seus valores são válidos.
    """
