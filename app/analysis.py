...
def analyze_desert_digging_data(main_farm_data: dict) -> dict:
    ...
    for dig in digging_data.get('grid', []):
        total_digs += 1
        x, y = dig.get('x'), dig.get('y')
        tool = dig.get('tool', 'Unknown Tool')
        tool_usage[tool] += 1

        # Pega o primeiro (e geralmente único) item encontrado no buraco
        item_name = next(iter(dig.get('items', {})), "Unknown")
        item_count = dig.get('items', {}).get(item_name, 0)
        items_found[item_name] += item_count

        if x is not None and y is not None and 0 <= x < 10 and 0 <= y < 10:
            grid_mirror[y][x] = {
                'item_name': item_name,
                'image': get_item_image_path(item_name)
            }
                ...
                if items_with_coords:
                    ...
                    for item in items_with_coords:
                        ...
                        if 0 <= cx < TARGET_GRID_SIZE and 0 <= cy < TARGET_GRID_SIZE:
                            mini_grid[cy][cx] = {'item_name': item['name'], 'image': get_item_image_path(item['name'])}
    ...
    if total_digs >= MIN_DIGS_FOR_HINTS:
        ...
        # D. Preparar dados para a máscara de regras (CORREÇÃO: Movido para fora do 'if treasure_anchors')
        rules_mask_data = {
        ...
        crab_hints_set = set()
        for (cx, cy) in crab_locations:
            has_treasure_neighbor = any(
                dug_items_map.get((cx + dx, cy + dy)) not in NON_TREASURE_ITEMS
                for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]
                if (cx + dx, cy + dy) in dug_items_map
            )
            if not has_treasure_neighbor:
                for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                    nx, ny = cx + dx, cy + dy
                    if 0 <= nx < 10 and 0 <= ny < 10 and (nx, ny) not in dug_items_map:
    ...
    return {
        ...
        'stats': {
            ...
            'items_found': dict(sorted(items_found.items())),
