...
# ==============================================================================
# CONSTANTES DE REGRAS DE NEGÓCIO PARA OBSIDIAN
# ==============================================================================
...
RESOURCE_NODE_MAP = {
    "lavaPits": {"name": "Obsidian", "recovery_name": "Lava Pit", "base_yield": 1, "recovery_time": 72 * 3600},
}
...
def _get_obsidian_buffs(farm_data: dict) -> list:
    """
    Coleta e filtra todos os bônus relevantes para a mineração de Obsidian
    e recuperação de Lava Pits.
    """
    ...
    obsidian_related_buffs = []
    obsidian_resource = "Obsidian"
    obsidian_recovery_resource = "Lava Pit"
    ...
    for buff in active_buffs:
        ...
        is_yield = buff.get("type") == "YIELD" and (
            (resource and resource == obsidian_resource) or
            (category and category == "Mineral") # Obsidian é um mineral
        )

        is_time = buff.get("type") in ["RECOVERY_TIME", "GROWTH_TIME"] and (
            (resource and resource == obsidian_recovery_resource) or
            (category and category == "Mineral")
        )

        if is_yield or is_time:
            obsidian_related_buffs.append(buff)

    bud_analysis = bud_service.analyze_bud_buffs(farm_data)
    if bud_analysis and "yield" in bud_analysis:
        if obsidian_resource in bud_analysis["yield"]:
            bud_yield_buff = bud_analysis["yield"][obsidian_resource]
            obsidian_related_buffs.append({
                "source_item": "Buds",
                "type": "YIELD",
                "operation": "add",
                "value": bud_yield_buff["total_boost"],
                "conditions": {"resource": obsidian_resource}
            })

    return obsidian_related_buffs

def _calculate_yield(base_yield: float, all_buffs: list, resource_name: str, node_context: dict = None) -> dict:
    """
    Calcula o rendimento final de Obsidian com base em todos os bônus.
    """
    ...
    yield_buffs = [b for b in all_buffs if b.get("type") == "YIELD" and (not b.get("conditions") or b.get("conditions", {}).get("resource") == resource_name or b.get("conditions", {}).get("category") == "Mineral")]
...
def _calculate_recovery_time(base_time: float, all_buffs: list, resource_name: str, node_context: dict = None) -> dict:
    """
    Calcula o tempo final de recuperação da Lava Pit com base em todos os bônus.
    """
...
def analyze_lava_resources(farm_data: dict) -> dict:
    """
    Analisa todas as Lava Pits (Obsidian), calcula bônus e retorna um relatório completo.
    """
    start_time = time.time()

    all_player_buffs = _get_obsidian_buffs(farm_data)
    ...
    critical_hit_stats = defaultdict(int) # Lava Pits não têm critical hits, mas mantemos a estrutura
    ...
    for api_key, node_info in RESOURCE_NODE_MAP.items():
        ...
        for node_id, node_data in nodes_api_data.items():
            ...
            critical_hits = {} # Lava Pits não têm critical hits
    ...
    end_time = time.time()
    log.info(f"Análise de Obsidian concluída em {end_time - start_time:.4f} segundos.")
